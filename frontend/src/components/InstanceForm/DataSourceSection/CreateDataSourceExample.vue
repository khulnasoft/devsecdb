  ...
  >
    ...
    >
      ...
      >
        ...
        <LearnMoreLink
          url="https://www.secdb.khulnasoft.com/docs/tutorials/local-mysql-instance?source=console"
          class="ml-1 text-sm"
        />
      </template>
...
CREATE OR REPLACE ROLE DEVSECDB;
...
-- Grant the least privileges required by Devsecdb 

GRANT CREATE DATABASE, EXECUTE TASK, IMPORT SHARE, APPLY MASKING POLICY, APPLY ROW ACCESS POLICY, APPLY TAG ON ACCOUNT TO ROLE DEVSECDB;

GRANT IMPORTED PRIVILEGES ON DATABASE SNOWFLAKE TO ROLE DEVSECDB;

GRANT USAGE ON WAREHOUSE "YOUR_COMPUTE_WAREHOUSE" TO ROLE DEVSECDB;

CREATE OR REPLACE USER DEVSECDB
  PASSWORD = 'YOUR_PWD'
  DEFAULT_ROLE = "DEVSECDB"
  DEFAULT_WAREHOUSE = "YOUR_COMPUTE_WAREHOUSE";

GRANT ROLE "DEVSECDB" TO USER DEVSECDB;

GRANT ROLE "DEVSECDB" TO ROLE SYSADMIN;

-- For each database to be managed by Devsecdb, you need to grant the following privileges

GRANT ALL PRIVILEGES ON DATABASE {{YOUR_DB_NAME}} TO ROLE DEVSECDB;

GRANT ALL PRIVILEGES ON ALL SCHEMAS IN DATABASE {{YOUR_DB_NAME}} TO ROLE DEVSECDB;
GRANT ALL PRIVILEGES ON FUTURE SCHEMAS IN DATABASE {{YOUR_DB_NAME}} TO ROLE DEVSECDB;

-- Then, grant the required schema object privileges on the database to the Devsecdb role
GRANT ALL PRIVILEGES ON ALL EVENT TABLES IN DATABASE {{YOUR_DB_NAME}} TO ROLE DEVSECDB;
GRANT ALL PRIVILEGES ON FUTURE EVENT TABLES IN DATABASE {{YOUR_DB_NAME}} TO ROLE DEVSECDB;

GRANT ALL PRIVILEGES ON ALL EXTERNAL TABLES IN DATABASE {{YOUR_DB_NAME}} TO ROLE DEVSECDB;
GRANT ALL PRIVILEGES ON FUTURE EXTERNAL TABLES IN DATABASE {{YOUR_DB_NAME}} TO ROLE DEVSECDB;

GRANT ALL PRIVILEGES ON ALL FILE FORMATS IN DATABASE {{YOUR_DB_NAME}} TO ROLE DEVSECDB;
GRANT ALL PRIVILEGES ON FUTURE FILE FORMATS IN DATABASE {{YOUR_DB_NAME}} TO ROLE DEVSECDB;

GRANT ALL PRIVILEGES ON ALL FUNCTIONS IN DATABASE {{YOUR_DB_NAME}} TO ROLE DEVSECDB;
GRANT ALL PRIVILEGES ON FUTURE FUNCTIONS IN DATABASE {{YOUR_DB_NAME}} TO ROLE DEVSECDB;

GRANT ALL PRIVILEGES ON ALL MATERIALIZED VIEWS IN DATABASE {{YOUR_DB_NAME}} TO ROLE DEVSECDB;
GRANT ALL PRIVILEGES ON FUTURE MATERIALIZED VIEWS IN DATABASE {{YOUR_DB_NAME}} TO ROLE DEVSECDB;

GRANT ALL PRIVILEGES ON ALL PROCEDURES IN DATABASE {{YOUR_DB_NAME}} TO ROLE DEVSECDB;
GRANT ALL PRIVILEGES ON FUTURE PROCEDURES IN DATABASE {{YOUR_DB_NAME}} TO ROLE DEVSECDB;

GRANT ALL PRIVILEGES ON ALL SEQUENCES IN DATABASE {{YOUR_DB_NAME}} TO ROLE DEVSECDB;
GRANT ALL PRIVILEGES ON FUTURE SEQUENCES IN DATABASE {{YOUR_DB_NAME}} TO ROLE DEVSECDB;

GRANT ALL PRIVILEGES ON ALL STAGES IN DATABASE {{YOUR_DB_NAME}} TO ROLE DEVSECDB;
GRANT ALL PRIVILEGES ON FUTURE STAGES IN DATABASE {{YOUR_DB_NAME}} TO ROLE DEVSECDB;

GRANT ALL PRIVILEGES ON ALL STREAMS IN DATABASE {{YOUR_DB_NAME}} TO ROLE DEVSECDB;
GRANT ALL PRIVILEGES ON FUTURE STREAMS IN DATABASE {{YOUR_DB_NAME}} TO ROLE DEVSECDB;

GRANT ALL PRIVILEGES ON ALL TABLES IN DATABASE {{YOUR_DB_NAME}} TO ROLE DEVSECDB;
GRANT ALL PRIVILEGES ON FUTURE TABLES IN DATABASE {{YOUR_DB_NAME}} TO ROLE DEVSECDB;

GRANT ALL PRIVILEGES ON ALL TASKS IN DATABASE {{YOUR_DB_NAME}} TO ROLE DEVSECDB;
GRANT ALL PRIVILEGES ON FUTURE TASKS IN DATABASE {{YOUR_DB_NAME}} TO ROLE DEVSECDB;

GRANT ALL PRIVILEGES ON ALL VIEWS IN DATABASE {{YOUR_DB_NAME}} TO ROLE DEVSECDB;
GRANT ALL PRIVILEGES ON FUTURE VIEWS IN DATABASE {{YOUR_DB_NAME}} TO ROLE DEVSECDB;
...
GRANT ALL PRIVILEGES ON ALL ALERTS IN DATABASE {{YOUR_DB_NAME}} TO ROLE DEVSECDB;
GRANT ALL PRIVILEGES ON FUTURE ALERTS IN DATABASE {{YOUR_DB_NAME}} TO ROLE DEVSECDB;

GRANT ALL PRIVILEGES ON ALL PASSWORD POLICYS IN DATABASE {{YOUR_DB_NAME}} TO ROLE DEVSECDB;
GRANT ALL PRIVILEGES ON FUTURE PASSWORD POLICYS IN DATABASE {{YOUR_DB_NAME}} TO ROLE DEVSECDB;

GRANT ALL PRIVILEGES ON ALL ROW ACCESS POLICYS IN DATABASE {{YOUR_DB_NAME}} TO ROLE DEVSECDB;
GRANT ALL PRIVILEGES ON FUTURE ROW ACCESS POLICYS IN DATABASE {{YOUR_DB_NAME}} TO ROLE DEVSECDB;

GRANT ALL PRIVILEGES ON ALL MASKING POLICYS IN DATABASE {{YOUR_DB_NAME}} TO ROLE DEVSECDB;
GRANT ALL PRIVILEGES ON FUTURE MASKING POLICYS IN DATABASE {{YOUR_DB_NAME}} TO ROLE DEVSECDB;

GRANT ALL PRIVILEGES ON ALL SESSION POLICYS IN DATABASE {{YOUR_DB_NAME}} TO ROLE DEVSECDB;
GRANT ALL PRIVILEGES ON FUTURE SESSION POLICYS IN DATABASE {{YOUR_DB_NAME}} TO ROLE DEVSECDB;

-- PIPE are not allowed to be bulk granted, you need to grant them one by one.
GRANT ALL PRIVILEGES ON PIPE {{PIPE_NAME}} IN DATABASE {{YOUR_DB_NAME}} TO ROLE DEVSECDB;
`;
...
-- If you need to create databases via Devsecdb
...
CREATE OR REPLACE ROLE DEVSECDB_READER;

-- If using non-enterprise edition, the following commands may encounter error likes 'Unsupported feature GRANT/REVOKE APPLY TAG', you can skip those unsupported GRANTs.
-- Grant the least privileges required by Devsecdb 

GRANT IMPORT SHARE TO ROLE DEVSECDB_READER;

GRANT IMPORTED PRIVILEGES ON DATABASE SNOWFLAKE TO ROLE DEVSECDB_READER;

GRANT USAGE ON WAREHOUSE "YOUR_COMPUTE_WAREHOUSE" TO ROLE DEVSECDB_READER;

CREATE OR REPLACE USER DEVSECDB_READER
  PASSWORD = 'YOUR_PWD'
  DEFAULT_ROLE = "DEVSECDB_READER"
  DEFAULT_WAREHOUSE = "YOUR_COMPUTE_WAREHOUSE";

GRANT ROLE "DEVSECDB_READER" TO USER DEVSECDB_READER;

GRANT ROLE "DEVSECDB_READER" TO ROLE SYSADMIN;

-- For each database to be managed by Devsecdb, you need to grant the following privileges

GRANT ALL PRIVILEGES ON DATABASE {{YOUR_DB_NAME}} TO ROLE DEVSECDB_READER;

GRANT ALL PRIVILEGES ON ALL SCHEMAS IN DATABASE {{YOUR_DB_NAME}} TO ROLE DEVSECDB_READER;
GRANT ALL PRIVILEGES ON FUTURE SCHEMAS IN DATABASE {{YOUR_DB_NAME}} TO ROLE DEVSECDB_READER;

-- Then, grant the required schema object privileges on the database to the Devsecdb role
GRANT ALL PRIVILEGES ON ALL EVENT TABLES IN DATABASE {{YOUR_DB_NAME}} TO ROLE DEVSECDB_READER;
GRANT ALL PRIVILEGES ON FUTURE EVENT TABLES IN DATABASE {{YOUR_DB_NAME}} TO ROLE DEVSECDB_READER;

GRANT ALL PRIVILEGES ON ALL EXTERNAL TABLES IN DATABASE {{YOUR_DB_NAME}} TO ROLE DEVSECDB_READER;
GRANT ALL PRIVILEGES ON FUTURE EXTERNAL TABLES IN DATABASE {{YOUR_DB_NAME}} TO ROLE DEVSECDB_READER;

GRANT ALL PRIVILEGES ON ALL FILE FORMATS IN DATABASE {{YOUR_DB_NAME}} TO ROLE DEVSECDB_READER;
GRANT ALL PRIVILEGES ON FUTURE FILE FORMATS IN DATABASE {{YOUR_DB_NAME}} TO ROLE DEVSECDB_READER;

GRANT ALL PRIVILEGES ON ALL FUNCTIONS IN DATABASE {{YOUR_DB_NAME}} TO ROLE DEVSECDB_READER;
GRANT ALL PRIVILEGES ON FUTURE FUNCTIONS IN DATABASE {{YOUR_DB_NAME}} TO ROLE DEVSECDB_READER;

GRANT ALL PRIVILEGES ON ALL MATERIALIZED VIEWS IN DATABASE {{YOUR_DB_NAME}} TO ROLE DEVSECDB_READER;
GRANT ALL PRIVILEGES ON FUTURE MATERIALIZED VIEWS IN DATABASE {{YOUR_DB_NAME}} TO ROLE DEVSECDB_READER;

GRANT ALL PRIVILEGES ON ALL PROCEDURES IN DATABASE {{YOUR_DB_NAME}} TO ROLE DEVSECDB_READER;
GRANT ALL PRIVILEGES ON FUTURE PROCEDURES IN DATABASE {{YOUR_DB_NAME}} TO ROLE DEVSECDB_READER;

GRANT ALL PRIVILEGES ON ALL SEQUENCES IN DATABASE {{YOUR_DB_NAME}} TO ROLE DEVSECDB_READER;
GRANT ALL PRIVILEGES ON FUTURE SEQUENCES IN DATABASE {{YOUR_DB_NAME}} TO ROLE DEVSECDB_READER;

GRANT ALL PRIVILEGES ON ALL STAGES IN DATABASE {{YOUR_DB_NAME}} TO ROLE DEVSECDB_READER;
GRANT ALL PRIVILEGES ON FUTURE STAGES IN DATABASE {{YOUR_DB_NAME}} TO ROLE DEVSECDB_READER;

GRANT ALL PRIVILEGES ON ALL STREAMS IN DATABASE {{YOUR_DB_NAME}} TO ROLE DEVSECDB_READER;
GRANT ALL PRIVILEGES ON FUTURE STREAMS IN DATABASE {{YOUR_DB_NAME}} TO ROLE DEVSECDB_READER;

GRANT ALL PRIVILEGES ON ALL TABLES IN DATABASE {{YOUR_DB_NAME}} TO ROLE DEVSECDB_READER;
GRANT ALL PRIVILEGES ON FUTURE TABLES IN DATABASE {{YOUR_DB_NAME}} TO ROLE DEVSECDB_READER;

GRANT ALL PRIVILEGES ON ALL TASKS IN DATABASE {{YOUR_DB_NAME}} TO ROLE DEVSECDB_READER;
GRANT ALL PRIVILEGES ON FUTURE TASKS IN DATABASE {{YOUR_DB_NAME}} TO ROLE DEVSECDB_READER;

GRANT ALL PRIVILEGES ON ALL VIEWS IN DATABASE {{YOUR_DB_NAME}} TO ROLE DEVSECDB_READER;
GRANT ALL PRIVILEGES ON FUTURE VIEWS IN DATABASE {{YOUR_DB_NAME}} TO ROLE DEVSECDB_READER;

-- ALERT and POLICY are the features in Snowflake Enterprise Edition, you can skip those GRANTs if you are using standard edition.
GRANT ALL PRIVILEGES ON ALL ALERTS IN DATABASE {{YOUR_DB_NAME}} TO ROLE DEVSECDB_READER;
GRANT ALL PRIVILEGES ON FUTURE ALERTS IN DATABASE {{YOUR_DB_NAME}} TO ROLE DEVSECDB_READER;

GRANT ALL PRIVILEGES ON ALL PASSWORD POLICYS IN DATABASE {{YOUR_DB_NAME}} TO ROLE DEVSECDB_READER;
GRANT ALL PRIVILEGES ON FUTURE PASSWORD POLICYS IN DATABASE {{YOUR_DB_NAME}} TO ROLE DEVSECDB_READER;

GRANT ALL PRIVILEGES ON ALL ROW ACCESS POLICYS IN DATABASE {{YOUR_DB_NAME}} TO ROLE DEVSECDB_READER;
GRANT ALL PRIVILEGES ON FUTURE ROW ACCESS POLICYS IN DATABASE {{YOUR_DB_NAME}} TO ROLE DEVSECDB_READER;

GRANT ALL PRIVILEGES ON ALL MASKING POLICYS IN DATABASE {{YOUR_DB_NAME}} TO ROLE DEVSECDB_READER;
GRANT ALL PRIVILEGES ON FUTURE MASKING POLICYS IN DATABASE {{YOUR_DB_NAME}} TO ROLE DEVSECDB_READER;

GRANT ALL PRIVILEGES ON ALL SESSION POLICYS IN DATABASE {{YOUR_DB_NAME}} TO ROLE DEVSECDB_READER;
GRANT ALL PRIVILEGES ON FUTURE SESSION POLICYS IN DATABASE {{YOUR_DB_NAME}} TO ROLE DEVSECDB_READER;

-- PIPE are not allowed to be bulk granted, you need to grant them one by one.
GRANT ALL PRIVILEGES ON PIPE {{PIPE_NAME}} IN DATABASE {{YOUR_DB_NAME}} TO ROLE DEVSECDB_READER;
`;
      case Engine.RISINGWAVE:
      case Engine.POSTGRES:
        if (
