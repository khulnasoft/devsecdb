...
const EXTERNAL_URL_PLACEHOLDER =
  "https://www.secdb.khulnasoft.com/docs/get-started/install/external-url";
const GITHUB_API_LIST_DEVSECDB_RELEASE =
  "https://api.github.com/repos/khulnasoft/devsecdb/releases";
...
export const useActuatorV1Store = defineStore("actuator_v1", {
  state: (): ActuatorState => ({
    ...
    releaseInfo: useLocalStorage("devsecdb_release", {
  ...
  getters: {
    changelogURL: (state) => {
      ...
      return `https://secdb.khulnasoft.com/changelog/devsecdb-${version.split(".").join("-")}/`;
    },
    info: (state) => {
    ...
    async fetchLatestRelease(): Promise<Release | undefined> {
      try {
        const { data: releaseList } = await useSilentRequest(() =>
          axios.get<Release[]>(`${GITHUB_API_LIST_DEVSECDB_RELEASE}?per_page=1`)
        );
