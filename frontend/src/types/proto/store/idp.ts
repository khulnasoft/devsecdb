...
export const OIDCIdentityProviderConfig: MessageFns<OIDCIdentityProviderConfig> = {
  encode(message: OIDCIdentityProviderConfig, writer: BinaryWriter = new BinaryWriter()): BinaryWriter {
    ...
    if (message.fieldMapping !== undefined) {
      FieldMapping.encode(message.fieldMapping, writer.uint32(34).fork()).join();
    }
    if (message.skipTlsVerify !== false) {
...
export const IdentityProviderUserInfo: MessageFns<IdentityProviderUserInfo> = {
  encode(message: IdentityProviderUserInfo, writer: BinaryWriter = new BinaryWriter()): BinaryWriter {
    if (message.identifier !== "") {
      writer.uint32(10).string(message.identifier);
    }
    if (message.displayName !== "") {
  ...
  decode(input: BinaryReader | Uint8Array, length?: number): IdentityProviderUserInfo {
    const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
    let end = length === undefined ? reader.len : reader.pos + length;
