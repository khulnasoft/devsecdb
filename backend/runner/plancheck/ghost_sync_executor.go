...
import (
	...
	"github.com/khulnasoft/devsecdb/backend/component/ghost"
...
func (e *GhostSyncExecutor) Run(ctx context.Context, config *storepb.PlanCheckRunConfig) (results []*storepb.PlanCheckRunResult_Result, err error) {
	...
	// It may be devsecdb who panicked, but that's rare. So
	...
	migrationContext, err := ghost.NewMigrationContext(ctx, rand.Intn(10000000), rand.Int63n(10000000), database, adminDataSource, e.secret, tableName, fmt.Sprintf("_dryrun_%d", time.Now().Unix()), renderedStatement, true, config.GhostFlags, 20000000)
	if err != nil {
