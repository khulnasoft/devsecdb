...
const (
	...
	ExternalURLPlaceholder = "https://www.secdb.khulnasoft.com/docs/get-started/install/external-url"
)
...
// TruncateStringWithDescription tries to truncate the string and append "... (view details in Devsecdb)" if truncated.
func TruncateStringWithDescription(str string) string {
	const limit = 450
	if truncatedStr, truncated := TruncateString(str, limit); truncated {
		return fmt.Sprintf("%s... (view details in Devsecdb)", truncatedStr)
	}
	return str
}
