...
import (
	...
	"github.com/khulnasoft/devsecdb/backend/resources/utils"
)
...
func initDB(pgBinDir, pgDataDir, pgUser string) error {
	...
	// it to the devsecdb user. Otherwise, initdb will complain file permission error.
	...
	if !sameUser {
		slog.Info(fmt.Sprintf("Recursively change owner of data directory %q to devsecdb...", pgDataDir))
		for _, dir := range dirListToChown {
			slog.Info(fmt.Sprintf("Change owner of %q to devsecdb", dir))
			if err := os.Chown(dir, int(uid), int(gid)); err != nil {
				return errors.Wrapf(err, "failed to change owner of %q to devsecdb", dir)
			}
		}
	}
