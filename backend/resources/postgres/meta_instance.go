...
// getPostgresDataDir returns the postgres data directory of Devsecdb.
func getPostgresDataDir(dataDir string, demoName string) string {
	...
	// we reset the demo data when starting Devsecdb and this can prevent accidentally removing the
	// production data.
	if demoName != "" {
