...
func getPreloadingUtilDir(utilDir string) string {
	...
	if strings.Contains(utilDir, "mysql") {
		...
		return fmt.Sprintf("/devsecdb/resources/mysqlutil-8.0.33-linux-%s", runtime.GOARCH)
	}
	if strings.Contains(utilDir, "mongoutil") {
		// we only support linux/amd64 or linux/arm64 now.
		return fmt.Sprintf("/devsecdb/resources/mongoutil-1.6.1-linux-%s", runtime.GOARCH)
	}
	if strings.Contains(utilDir, "postgres") {
		// we only support linux/amd64 or linux/arm64 now.
		return fmt.Sprintf("/devsecdb/resources/postgres-linux-%s-16", runtime.GOARCH)
	}
	return utilDir
}
