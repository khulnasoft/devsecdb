...
type Profile struct {
	...
	// ExternalURL is the URL user visits Devsecdb.
	ExternalURL string
	// DatastorePort is the binding port for database instance for storing Devsecdb metadata.
	...
	Port int
	// PgUser is the user we use to connect to devsecdb's Postgres database.
	...
	Debug bool
	// DataDir is the directory stores the data including Devsecdb's own database, backups, etc.
	DataDir string
	...
	// Version is the devsecdb's server version
	Version string
	...
	LastActiveTs int64
	// Unique ID per Devsecdb instance run.
	DeployID string
