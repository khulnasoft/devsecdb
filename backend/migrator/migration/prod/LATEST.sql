...
CREATE TABLE instance_change_history (
    ...
    updater_id INTEGER NOT NULL REFERENCES principal (id),
    updated_ts BIGINT NOT NULL DEFAULT extract(epoch from now()),
    -- NULL means the migrations for Devsecdb's own metadata database.
    ...
    -- Record the client version creating this change history. For Devsecdb, we use its binary release version. Different Devsecdb release might
