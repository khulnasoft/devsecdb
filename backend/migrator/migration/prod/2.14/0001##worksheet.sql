...
-- Migrate sheet with DEVSECDB source to the worksheet table.
...
SELECT
    ...
    sheet.payload
FROM sheet
WHERE sheet.source = 'DEVSECDB';

-- Change fk for sheet_organizer.
DELETE FROM sheet_organizer USING sheet WHERE sheet_organizer.sheet_id = sheet.id AND sheet.source != 'DEVSECDB';
...
DELETE FROM sheet WHERE sheet.source = 'DEVSECDB';
