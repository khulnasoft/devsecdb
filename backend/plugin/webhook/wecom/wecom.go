...
import (
	...
	"github.com/khulnasoft/devsecdb/backend/plugin/webhook"
)
...
func getMessageCard(context webhook.Context) *WebhookMarkdown {
	...
	content := fmt.Sprintf("# %s%s\n\n%s\n[View in Devsecdb](%s)", status, context.Title, strings.Join(metaStrList, "\n"), context.Link)
	if context.Description != "" {
		content = fmt.Sprintf("# %s%s\n> %s\n\n%s\n[View in Devsecdb](%s)", status, context.Title, context.Description, strings.Join(metaStrList, "\n"), context.Link)
	}
