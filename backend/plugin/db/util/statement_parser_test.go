...
func TestSanitizeQuery(t *testing.T) {
	tests := []struct {
		...
		{
			input: `
-- This is the devsecdb schema to track migration info for Spanner
-- Create a database called devsecdb in the driver.
-- CREATE DATABASE devsecdb;
...
CREATE TABLE migration_history (
    ...
    -- Record the client version creating this migration history. For Devsecdb, we use its binary release version. Different Devsecdb release might
    ...
    -- Since devsecdb also manages different application databases from an instance, it leverages this field to track each database migration history.
...
CREATE UNIQUE INDEX devsecdb_idx_unique_migration_history_namespace_sequence ON migration_history (namespace, sequence);

CREATE UNIQUE INDEX devsecdb_idx_unique_migration_history_namespace_version ON migration_history (namespace, version);

CREATE INDEX devsecdb_idx_migration_history_namespace_source_type ON migration_history(namespace, source, type);

CREATE INDEX devsecdb_idx_migration_history_namespace_created ON migration_history(namespace, created_ts);
      `,
			want: []string{
				`CREATE TABLE migration_history (
