...
import (
	...
	pgparser "github.com/khulnasoft/devsecdb/backend/plugin/parser/pg"
...
func getMaterializedViews(txn *sql.Tx) (map[string][]*storepb.MaterializedViewMetadata, error) {
	...
	for rows.Next() {
		...
		if !def.Valid {
			return nil, errors.Errorf("schema %q materialized view %q has empty definition; please check whether proper privileges have been granted to Devsecdb", schemaName, matview.Name)
		}
...
func getViews(txn *sql.Tx, columnMap map[db.TableKey][]*storepb.ColumnMetadata) (map[string][]*storepb.ViewMetadata, error) {
	...
	for rows.Next() {
		...
		// Return error on NULL view definition.
		// https://github.com/khulnasoft/devsecdb/issues/343
		if !def.Valid {
			return nil, errors.Errorf("schema %q view %q has empty definition; please check whether proper privileges have been granted to Devsecdb", schemaName, view.Name)
		}
