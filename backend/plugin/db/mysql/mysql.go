...
func (d *Driver) Execute(ctx context.Context, statement string, opts db.ExecuteOptions) (int64, error) {
	...
	if err := conn.Raw(func(driverConn any) error {
		...
		for i, command := range commands {
			...
			sqlWithDevsecdbAppComment := util.MySQLPrependDevsecdbAppComment(command.Text)
			sqlResult, err := exer.ExecContext(ctx, sqlWithDevsecdbAppComment, nil)
			if err != nil {
...
func (d *Driver) QueryConn(ctx context.Context, conn *sql.Conn, statement string, queryContext db.QueryContext) ([]*v1pb.QueryResult, error) {
	...
	for _, singleSQL := range singleSQLs {
		...
		sqlWithDevsecdbAppComment := util.MySQLPrependDevsecdbAppComment(statement)
		...
		queryResult, err := func() (*v1pb.QueryResult, error) {
			if allQuery {
				rows, err := conn.QueryContext(ctx, sqlWithDevsecdbAppComment)
				if err != nil {
					return nil, err
				}
				defer rows.Close()
