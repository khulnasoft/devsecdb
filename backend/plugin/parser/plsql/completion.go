...
func (m CompletionMap) insertDatabases(c *Completer) {
	for _, name := range c.listAllDatabases() {
		m.Insert(base.Candidate{
			Type: base.CandidateTypeSchema, // For oracle, schema is the same as database modified by devsecdb.
			Text: name,
		})
	}
}
