...
import (
	...
	parsererror "github.com/khulnasoft/devsecdb/backend/plugin/parser/errors"
...
func (q *querySpanExtractor) getRangeVarsFromJSONRecursive(jsonData map[string]any, currentDatabase, currentSchema string) ([]base.ColumnResource, error) {
	...
	if jsonData["RangeVar"] != nil {
		...
		// Devsecdb do not sync the system objects, so we skip finding for system objects in the metadata.
