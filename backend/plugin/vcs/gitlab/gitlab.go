...
func (p *Provider) fetchPaginatedRepositoryList(ctx context.Context, page int) (repos []Repository, hasNextPage bool, err error) {
	...
	// https://github.com/khulnasoft/devsecdb/pull/1423#discussion_r884278534 for the
	// discussion.
...
func (p *Provider) CreateWebhook(ctx context.Context, repositoryID string, payload []byte) (string, error) {
	...
	if code != http.StatusCreated {
		...
		// Add helper tips if the status code is 422, refer to https://github.com/khulnasoft/devsecdb/issues/101 for more context.
		if code == http.StatusUnprocessableEntity {
			reason += ".\n\nIf GitLab and Devsecdb are in the same private network, " +
