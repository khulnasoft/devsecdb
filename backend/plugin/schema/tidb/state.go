...
import (
	...
	"github.com/khulnasoft/devsecdb/backend/plugin/db/tidb"
	tidbparserraw "github.com/khulnasoft/devsecdb/backend/plugin/parser/tidb"
...
func (p *partitionState) convertToPartitionMetadata() []*storepb.TablePartitionMetadata {
	...
	if p.info.useDefault != 0 {
		generator := tidbparserraw.NewPartitionDefaultNameGenerator("")
		...
		for _, partition := range sortedPartitions {
			...
			if p.subInfo != nil {
				if p.subInfo.useDefault != 0 {
					...
					generator := tidbparserraw.NewPartitionDefaultNameGenerator(partition.name)
...
func getDataTypePlainText(typeCtx tidbparser.IDataTypeContext) string {
