...
import (
	...
	"github.com/khulnasoft/devsecdb/backend/plugin/vcs/azure"
	"github.com/khulnasoft/devsecdb/backend/plugin/vcs/bitbucket"
	"github.com/khulnasoft/devsecdb/backend/plugin/vcs/github"
...
func createVCSWebhook(ctx context.Context, vcsProvider *store.VCSProviderMessage, webhookEndpointID, webhookSecretToken, externalRepoID, devsecdbEndpointURL string) (string, error) {
	...
	case storepb.VCSType_GITLAB:
		...
		webhookCreate := gitlab.WebhookCreate{
			URL:                   fmt.Sprintf("%s/hook/%s", devsecdbEndpointURL, webhookEndpointID),
