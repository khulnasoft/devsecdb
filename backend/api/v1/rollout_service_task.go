...
func getCreateDatabaseStatement(dbType storepb.Engine, c *storepb.PlanConfig_CreateDatabaseConfig, databaseName, adminDatasourceUser string) (string, error) {
	...
	case storepb.Engine_POSTGRES:
		...
		// We need to grant the database owner role to the data source admin so that Devsecdb can have permission for the database using the data source admin.
	...
	case storepb.Engine_MONGODB:
		...
		// And we pass the database name to Devsecdb engine driver, which will be used to build the connection string.
