...
import (
	...
	"github.com/khulnasoft/devsecdb/backend/plugin/mail"
	"github.com/khulnasoft/devsecdb/backend/plugin/schema"
	"github.com/khulnasoft/devsecdb/backend/plugin/webhook/feishu"
	"github.com/khulnasoft/devsecdb/backend/plugin/webhook/slack"
	"github.com/khulnasoft/devsecdb/backend/plugin/webhook/wecom"
...
func (s *SettingService) sendTestEmail(ctx context.Context, value *v1pb.SMTPMailDeliverySettingValue) error {
	...
	consoleRedirectURL := "www.secdb.khulnasoft.com"
	...
	email.SetFrom(fmt.Sprintf("Devsecdb <%s>", value.From)).AddTo(value.To).SetSubject("Devsecdb mail server test").SetBody(polishHTMLBody)
...
func prepareTestMailContent(htmlTemplate, consoleRedirectURL, logoContentID, bannerContentID string) string {
	testEmailContent := strings.ReplaceAll(htmlTemplate, "{{DEVSECDB_LOGO_URL}}", fmt.Sprintf("cid:%s", logoContentID))
	testEmailContent = strings.ReplaceAll(testEmailContent, "{{DEVSECDB_BANNER_URL}}", fmt.Sprintf("cid:%s", bannerContentID))
	testEmailContent = strings.ReplaceAll(testEmailContent, "{{DEVSECDB_CONSOLE_REDIRECT_URL}}", consoleRedirectURL)
