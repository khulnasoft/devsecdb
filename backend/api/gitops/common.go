...
const (
	commentPrefixDevsecdbBot     = "**[Devsecdb Bot]**"
	commentPrefixSQLReview       = "**[Devsecdb SQL Review]**"
	commentPrefixSQLReviewPassed = "SQL Review Check Passed"
)

func getPullRequestComment(externalURL, issue string) string {
	return fmt.Sprintf("This pull request has triggered a Devsecdb rollout 🚀. Check out the status at %s/%s", externalURL, issue)
}
