...
import (
	...
	v1api "github.com/khulnasoft/devsecdb/backend/api/v1"
...
func (s *Service) RegisterDirectorySyncRoutes(g *echo.Group) {
	...
	// Get a single user. The user id is the Devsecdb user uid.
	...
	// Get a single group. The group id is the Devsecdb group resource id.
		...
		for _, op := range patch.Operations {
			...
			case "members":
				...
				for _, value := range values {
					...
					// the member identifier in group patch is Devsecdb user uid
