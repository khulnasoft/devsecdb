...
Currently, Devsecdb users can set up a backup policy, take full logical backups automatically following the backup policy schedule, and restore the backup to a new database. There are two problems we would like to address in this design.
...
- The remaining disk storage should be enough. Devsecdb will estimate the database dump size, and make sure that there is always at lease 1GB more disk space.
...
As long as the instance fulfills all the [requirements](#requirements) and enables a backup policy in the Devsecdb environment that contains this instance, the user is lucky enough to have a PITR option. The user could then choose those databases they want to recover, and select the point of time, then Devsecdb will recover the databases to precisely the state of that point of time.

What’s more convenient is that if the user used Devsecdb to do the wrong schema migration, then Devsecdb will record the exact time before the schema migration and provide a one-click Point-in-time Recovery experience.
...
1. `FLUSH TABLES tbl_names... WITH READ LOCK`. Devsecdb will set a timeout to release the lock if things go wrong.
2. `SHOW MASTER STATUS`. Devsecdb will record the binary log coordinates and timestamp.
...
The logical backup and data changes, i.e., binlog, should be kept in safe and durable storage, e.g., AWS S3 or NFS. Note that users should not manually manage the storage, introducing inconsistency between the metadata in Devsecdb and storage contents.

Currently, we keep the logical backup and binlog together in the local disk where Devsecdb resides. We may integrate with cloud storage in the long term.

The partial backup dump should be discarded if Devsecdb encounters an unrecoverable error during the full backup process. The system should report a failed backup, and the user could choose to start another backup task manually. The partially dumped logical backup file will be automatically deleted by Devsecdb.

To check the integrity of the logical dump, Devsecdb will save file size in the backup metadata. When using a logical backup to recover, we should first validate the file size of the dump file and the metadata matches.
...
The process above should be executed every minute. When a download task fails, Devsecdb should delete the corresponding binlog file. The downloading process must be idempotent.

To accomplish the tasks described above, Devsecdb will embed the newest version of `mysqlbinlog` binary, which is also used in the recovery process.

When the backup is enabled for a specific database, Devsecdb will check where the PITR requirements are met and automatically upgrade to PITR and download the binlog files.
...
`mysqlbinlog mysql-bin.000001 --ssl=0 --read-from-remote-server --host=x --user=x --password=x --raw --result-file=/devsecdb/binlog/`

## Find the point-in-time T

The user should be able to navigate the database history to search for SQL queries to determine the exact point in time to recover. As is described in the [OMG Moments](#omg-moments), when the user meets a schema migration failure, Devsecdb should locate the binlog position just before the corresponding migration happens.

Devsecdb will support a binlog search api accepting a timestamp and returns several numbers of events around it, containing the data changes and binlog positions. The user should choose a proper binlog position from the UI so that Devsecdb can recover the database to the corresponding state.

MySQL binlog contains information about the ALTER TABLE statement. When a schema migration happens in Devsecdb, we should search the binlog and find the position of the schema migration. If the user wants to revert the schema migration, this position should be the target position of a later PITR. The user can revert it with a simple button click.

## Recovery

When the point-in-time t1 is determined, Devsecdb will take a series of actions to accomplish the task. The recovery plan is as follows.
...
Step 6b uses a special MySQL syntax [`RENAME TABLE current_db.tbl_name TO other_db.tbl_name`](https://dev.mysql.com/doc/refman/8.0/en/rename-table.html), which effectively moves tables between databases. At last, Devsecdb will delete the pitr database, which will be empty. The original database, if it exists, will be deleted if the user manually approves the issue Devsecdb automatically generates.

After the recovery process described above, Devsecdb will ask for the user’s manual approval to delete the old database.

To restore binlog from the original to the pitr table as is described in step 4, Devsecdb will use the [`--rewrite-db`](https://dev.mysql.com/doc/refman/8.0/en/mysqlbinlog.html#option_mysqlbinlog_rewrite-db) option. An example will be `mysqlbinlog --rewrite-db='dbtarget->dbtarget_pitr' --database=dbtarget_pitr binlog.00001`. The `--database` option filters changed database names, so should not use the original database name.
...
Before the cutover stage, Devsecdb will do several checks to prevent potential damage:
...
Devsecdb should make some changes to the API and schema design to implement the PITR feature.
...
When the user performs a PITR recovery, Devsecdb will check the api.Backup and try to find the PITR information. If the PITR information is not empty, Devsecdb will perform a binlog replay after recovering the logical backup using the [recovery process described above](#recovery).
...
Devsecdb will currently implement a default data retention policy of 7 days. We may later make this configurable as an improvement.


# Monitoring

Steps 3 and 4 could take some time, and Devsecdb should export the progress via api.Task, which could be obtained from an api.Issue.


# Third-Party Integration

Devsecdb should provide event webhooks for user system integration. For example, we could send the logical backup success/failure events to a user-defined URL.
